-- Basalt 3.0.0-dev (single-file bundle)
-- Generated by bundle.lua - do not edit, edit src/ instead.
local sources = {}
sources["main"] = [=[
...
if type(require) ~= "function" then
    local name, path = ...
    local initPath = fs.combine(fs.getDir(path or "Basalt/src/main.lua"), "init.lua")
...
function basalt.run()
    if running then error("Basalt3 is already running", 2) end
    running = true
            ...
            if prop then
                ...
                if old ~= v then
                    ...
                    if (type(v) == "function" and not prop.rawFunction)
                        or state.is(v) then
                        if prop.visual then t:markDirty() end
                    else
...
function class.combinedProperty(c, combinedName, propertyNames)
    if type(combinedName) ~= "string" or combinedName == "" then
        error("Basalt3 class: combined property name must be a non-empty string", 2)
    end
    if type(propertyNames) ~= "table" or #propertyNames == 0 then
        error("Basalt3 class: combined property list must not be empty", 2)
    end
    local names = {}
    for i = 1, #propertyNames do
        local propName = propertyNames[i]
        if type(propName) ~= "string" or c.__props[propName] == nil then
            error("Basalt3 class: unknown property '" .. tostring(propName)
    ...
    c[setterName] = function(self, ...)
        ...
        if values.n ~= #names then
            error("Basalt3: " .. setterName .. " expects " .. #names
...
local function replaceItems(self, values, oldItems)
    if type(values) ~= "table" then
        error("Basalt3 Collection: items must be a table", 3)
    end
...
local function descendantBox(container, el)
    ...
    if p ~= container then
        error("Basalt3 scroll: element is not a descendant of this container", 3)
    end
...
function Element:setState(stateName, active)
    if type(stateName) ~= "string" or stateName == "" then
        error("Basalt3: state name must be a non-empty string", 2)
    end
...
function Element:setStateStyle(stateName, props, priority)
    if type(props) ~= "table" then
        error("Basalt3: state style must be a table", 2)
    end
    ...
    for propName, value in pairs(props) do
        ...
        if not prop then
            error("Basalt3: unknown state style property '" .. tostring(propName) .. "'", 2)
        end
        if not prop.styleable then
            error("Basalt3: property '" .. propName .. "' cannot be state-styled", 2)
        end
...
function Element:setStatePriority(stateName, priority)
    if type(priority) ~= "number" then
        error("Basalt3: state priority must be a number", 2)
    end
...
function Element:bind(propName, source, options)
    ...
    if not c.__props[propName] then
        error("Basalt3: cannot bind unknown property '" .. tostring(propName) .. "'", 2)
    end
    if not state.is(source) then
        error("Basalt3: bind expects a state or computed value", 2)
    end
    ...
    if type(options) ~= "table" then
        error("Basalt3: bind options must be a function or table", 2)
    end
    ...
    if options.fromState then
        ...
        if type(transform) ~= "function" then
            error("Basalt3: fromState must be a function", 2)
        end
        local el = self
    ...
    if twoWay then
        ...
        if toState ~= nil and type(toState) ~= "function" then
            error("Basalt3: toState must be a function", 2)
        end
...
function errors.show(err, trace, showTrace)
    ...
    cprint("Basalt3 Error", colors.red)
    print()
...
function itemview.geometry(count, rows, offset, mode)
    if mode ~= "auto" and mode ~= "always" and mode ~= "hidden" then
        error("Basalt3 item scrollbar: expected 'auto', 'always' or 'hidden'", 3)
    end
...
function layout.fill(weight)
    ...
    if type(weight) ~= "number" or weight <= 0 then
        error("Basalt3 layout: fill weight must be greater than zero", 2)
    end
    return token("fill", weight)
end
function layout.percent(amount)
    if type(amount) ~= "number" or amount < 0 then
        error("Basalt3 layout: percent must be a non-negative number", 2)
    end
...
local function parse(r, g, b)
    if type(r) == "string" then
        ...
        elseif not hex:match("^%x%x%x%x%x%x$") then
            error("Basalt3: invalid color string '" .. r
    ...
    if g == nil then
        if r < 0 or r > 0xFFFFFF or r % 1 ~= 0 then
            error("Basalt3: invalid color number "
...
function palette.rgb(r, g, b)
    ...
    if regCount > 255 then
        error("Basalt3: color registry is full (max 240 custom colors)", 2)
    end
    local idx = regCount
    regCount = regCount + 1
    reg[idx] = { rr, gg, bb }
...
function reactive.compile(str, element)
    ...
    if not fn then
        error("Basalt3: invalid reactive expression " .. str
            .. ": " .. tostring(err), 3)
    end
    return fn
end
...
local function colorChar(color)
    ...
    if not c then
        error("Basalt3: unknown color value " .. tostring(color)
...
local function barMode(self)
    ...
    if mode ~= "auto" and mode ~= "always" and mode ~= "hidden" then
        error("Basalt3 scroll: scrollbar must be 'auto', 'always' or 'hidden'", 3)
    end
    return mode
end
...
function Signal:update(fn)
    if type(fn) ~= "function" then
        error("Basalt3 state: update expects a function", 2)
    end
...
function Signal:subscribe(fn, immediate)
    if type(fn) ~= "function" then
        error("Basalt3 state: subscribe expects a function", 2)
    end
...
function Signal:map(fn)
    if type(fn) ~= "function" then
        error("Basalt3 state: map expects a function", 2)
    end
    local source = self
...
function state.computed(fn)
    if type(fn) ~= "function" then
        error("Basalt3 computed: expected a function", 2)
    end
...
function Flex:measure(availableWidth, availableHeight)
    ...
    if not isRow and direction ~= "column" then
        error("Basalt3 layout: direction must be 'row' or 'column'", 2)
    end
    local padding = math.max(0, math.floor(tonumber(self.padding) or 0))
...
function Flex:layoutChildren()
    ...
    if self.overflow ~= "clip" then
        error("Basalt3 layout: only overflow='clip' is currently supported", 2)
    end
    ...
    elseif justify ~= "start" then
        error("Basalt3 layout: invalid justify '" .. tostring(justify) .. "'", 2)
    end
    ...
    for i = 1, #items do
        ...
        elseif align ~= "start" and align ~= "stretch" then
            error("Basalt3 layout: invalid align '" .. tostring(align) .. "'", 2)
        end
...
function Program:execute(path, ...)
    ...
    if not resolved then
        error("Basalt3 Program: program not found: " .. tostring(path), 2)
    end
    local root = self:getRoot()
    local parentTerm = rawget(root, "term")
    if not parentTerm then
        error("Basalt3 Program: element must be inside a frame before execute()", 2)
    end
    ...
    if not fn then
        error("Basalt3 Program: failed to load " .. resolved .. ": " .. tostring(err), 2)
    end
    local proc = {
        window = win,
        term = win,
...
local function normalizeColumns(columns)
    if type(columns) ~= "table" then
        error("Basalt3 Table: columns must be a table", 3)
    end
    local result = {}
    for i, column in ipairs(columns) do
        ...
        elseif type(column) == "table" then
            ...
            error("Basalt3 Table: column " .. i .. " must be a string or table", 3)
        end
    end
    return result
end
...
function Table:setSortDirection(direction)
    if direction ~= "asc" and direction ~= "desc" then
        error("Basalt3 Table: sortDirection must be 'asc' or 'desc'", 2)
    end
...
function Table:setData(rawData, formatters)
    if type(rawData) ~= "table" then
        error("Basalt3 Table: data must be a table", 2)
    end
    self:clear()
...
class.property(Tree, "nodes", false, {
    onChange = function(self, nodes)
        if type(nodes) ~= "table" then
            error("Basalt3 Tree: nodes must be a table", 3)
        end
...
function Tree:setExpandedNodes(expanded)
    if type(expanded) ~= "table" then
        error("Basalt3 Tree: expandedNodes must be a table", 2)
    end
    local function walk(nodes)
        for _, node in ipairs(nodes) do
...
function animation.to(el, props, duration, easing, onDone)
    local a = {
        ...
        easing = easings[easing or "easeInOut"]
            or error("Basalt3 animation: unknown easing '" .. tostring(easing) .. "'", 2),
        onDone = onDone,
    }
    for prop in pairs(props) do
        local v = el[prop]
        if type(v) ~= "number" then
            error("Basalt3 animation: property '" .. prop .. "' is not a number", 2)
        end
        a.from[prop] = v
    end
    active[#active + 1] = a
...
class.property(BigFont, "fontSize", 1, {
    onChange = function(self, size)
        if size < 1 or size > 3 or size % 1 ~= 0 then
            error("Basalt3 bigfont: fontSize must be 1, 2 or 3", 3)
        end
    end,
})
...
function Graph:addPoint(name, value)
    local series = self:getSeries(name)
    if not series then
        error("Basalt3 charts: unknown series '" .. tostring(name) .. "'", 2)
    end
...
function image.load(path)
    ...
    if not handle then
        error("Basalt3 image: cannot open " .. tostring(path), 2)
    end
    local content = handle.readAll()
    handle.close()
    local bimg = textutils.unserialize(content)
    if type(bimg) ~= "table" then
        error("Basalt3 image: " .. path .. " is not a valid bimg file", 2)
    end
    return bimg
end
...
function responsive.apply(element, rules)
    if type(rules) ~= "table" then
        error("Basalt3 responsive: rules must be a table", 2)
    end
    if not element.getChildren then
        error("Basalt3 responsive: target must be a container", 2)
    end
    ...
    for i = 1, #rules do
        ...
        if type(rule) ~= "table" then
            error("Basalt3 responsive: rule " .. i .. " must be a table", 2)
        end
        if rule.when ~= nil and type(rule.when) ~= "function" then
            error("Basalt3 responsive: rule.when must be a function", 2)
        end
        if rule.props ~= nil and type(rule.props) ~= "table" then
            error("Basalt3 responsive: rule.props must be a table", 2)
        end
        if rule.name then
            if type(rule.name) ~= "string" or rule.name == "" then
                error("Basalt3 responsive: rule.name must be a non-empty string", 2)
            end
            if names[rule.name] then
                error("Basalt3 responsive: duplicate state name '" .. rule.name .. "'", 2)
            end
...
function theme.set(themeTable)
    for typeName, props in pairs(themeTable) do
        ...
        if not cls then
            error("Basalt3 theme: unknown element type '" .. typeName .. "'", 2)
        end
        for k, v in pairs(props) do
            if k == "states" then
                if type(v) ~= "table" then
                    error("Basalt3 theme: states for " .. typeName
                        .. " must be a table", 2)
                end
            ...
            elseif cls.__props[k] == nil then
                error("Basalt3 theme: unknown property '" .. k
                    .. "' for " .. typeName, 2)
            elseif type(v) == "string" and v:sub(1, 1) == "{" then
                error("Basalt3 theme: reactive strings are not allowed in "
...
function xml.parse(src)
    ...
    while true do
        ...
        if src:sub(lt + 1, lt + 3) == "!--" then
            ...
            if not close then error("Basalt3 XML: unclosed comment", 2) end
            pos = close + 3
        elseif src:sub(lt + 1, lt + 1) == "/" then
            local gt = src:find(">", lt, true)
            if not gt then error("Basalt3 XML: malformed closing tag", 2) end
            local tagName = trim(src:sub(lt + 2, gt - 1))
            local top = stack[#stack]
            if top.tag ~= tagName then
                error("Basalt3 XML: unexpected </" .. tagName .. ">"
            ...
            local gt = src:find(">", lt, true)
            if not gt then error("Basalt3 XML: unclosed tag", 2) end
            ...
            if not tagName then error("Basalt3 XML: malformed tag near pos " .. lt, 2) end
    ...
    if #stack ~= 1 then
        error("Basalt3 XML: unclosed <" .. stack[#stack].tag .. ">", 2)
    end
...
local function build(parent, nodes, scope)
    ...
    for _, node in ipairs(nodes) do
        ...
        if not add then
            error("Basalt3 XML: unknown element <" .. node.tag .. ">", 2)
        end
        local el = add(parent)
        for k, v in pairs(node.attrs) do
            if k:find("^on%u") then
                ...
                if type(fn) ~= "function" then
                    error("Basalt3 XML: scope has no handler '" .. v
                        .. "' for " .. k .. " on <" .. node.tag .. ">", 2)
                end
                el[k](el, fn)
            else
...
function xml.loadFile(parent, path, scope)
    ...
    if not h then error("Basalt3 XML: cannot open " .. path, 2) end
    local src = h.readAll()
    h.close()
...
local function loader(name)
    ...
    local source = sources[name]
        or error("Basalt3: module not bundled: " .. tostring(name), 0)
